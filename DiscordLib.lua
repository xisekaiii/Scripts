...
tag = tostring(math.random(0001, 9999))
...
local Quartyz = Instance.new("ScreenGui")
Quartyz.Name = "Quartyz GUI"
Quartyz.Parent = game:GetService("CoreGui")
Quartyz.ZIndexBehavior = Enum.ZIndexBehavior.Sibling

local NotiScreen = Instance.new("ScreenGui")
NotiScreen.Name = "Quartyz Notification"
NotiScreen.Parent = game:GetService("CoreGui")
NotiScreen.ZIndexBehavior = Enum.ZIndexBehavior.Sibling

function QuartyzLib:Window(text,mainclr,cls)
	...
	UserInputService.InputBegan:Connect(function(input)
		if input.KeyCode == CloseBind then
			if uitoggled == false then
				...
				uitoggled = true
				Quartyz.Enabled = false
			else
				Quartyz.Enabled = true
	...
	OutLayFrame.BackgroundTransparency = 1
	...
	function QuartyzLib:Notification(titletext, desctext, btntext)
		local textcon = btntext or "OK"
	...
	function ServerHold:Server(text, img)
		...
		function ChannelHold:Channel(text,ico)
			...
			function ChannelContent:Toggle(text,default,callback)
				...
				Toggle.Position = UDim2.new(0.261979163, 0, 0.210789461, 0)
				...
				TextButton_Toggle.Size = UDim2.new(0, 401, 0, 30)
				...
				TextButton_2_Toggle.BackgroundColor3 = Color3.fromRGB(21, 21, 21)
				TextButton_2_Toggle.BorderColor3 = Color3.fromRGB(20, 20, 20)
				TextButton_2_Toggle.BorderSizePixel = 0
				TextButton_2_Toggle.Position = UDim2.new(0.92, 0, 0.15, 0)
				...
				ImageButton.ImageRectOffset = Vector2.new(644, 204)
				ImageButton.ImageRectSize = Vector2.new(36, 36)
				ImageButton.ImageColor3 = PresetColor
				...
				TextLabel_Toggle.Position = UDim2.new(0, 15, 0, 1)
				TextLabel_Toggle.Size = UDim2.new(0, 400, 0, 30)
				TextLabel_Toggle.Font = Enum.Font.SourceSans
				TextLabel_Toggle.TextColor3 = PresetColor
				TextLabel_Toggle.TextSize = 17.000
				TextLabel_Toggle.Text = text
				TextLabel_Toggle.TextWrapped = true
				TextLabel_Toggle.TextXAlignment = Enum.TextXAlignment.Left

				if default == true then
					ImageButton:TweenSizeAndPosition(UDim2.new(0, 25, 0, 25), UDim2.new(0, 0, 0, 0), "In", "Bounce", 0.1, true)
					wait(0.1)
					ImageButton:TweenSizeAndPosition(UDim2.new(0, 21, 0, 21), UDim2.new(0, 0, 0, 0), "In", "Bounce", 0.1, true)
					TweenService:Create(
						Main_UiStroke,
				...
				TextButton_Toggle.MouseButton1Click:Connect(function()
					if toggled == false then
						...
						ImageButton:TweenSizeAndPosition(UDim2.new(0, 25, 0, 25), UDim2.new(0, 0, 0, 0), "In", "Bounce", 0.1, true)
						wait(0.1)
						ImageButton:TweenSizeAndPosition(UDim2.new(0, 0, 0, 0), UDim2.new(0, 0, 0, 0), "In", "Bounce", 0.1, true)
						TweenService:Create(
							Main_UiStroke,
							TweenInfo.new(.2, Enum.EasingStyle.Bounce, Enum.EasingDirection.Out,0.1),
							{Color = PresetColor}
			...
			function ChannelContent:Dropdown(text, list, callback)
				...
				DropdownFrameMainOutline.Position = UDim2.new(-0.00155700743, 0, 2.26983342, 0)
				...
				DropdownFrameMain.Position = UDim2.new(0.00899999978, 0, 2.3368965, 0)
				...
				TextBox.FocusLost:Connect(function(ep)
					...
					if ep then
						if #TextBox.Text > 0 then
							...
							if disapper then
								TextBox.Text = TextboxTitle.Text.." : "..TextBox.Text
							end
						end
					end
				end)
...
local win = QuartyzLib:Window("Mukuro")

local serv = win:Server("Preview", "")

local btns = serv:Channel("Button", "http://www.roblox.com/asset/?id=7040391851")
...
btns:Button("Get max level", function()
	QuartyzLib:Notification("Notification", "Max level!", "Okay!")
end)
